 ...
 *
 * SIN CLAVES FORÁNEAS, a propósito. La base de datos de producción nunca las
 * ha tenido: MITECO publica datos que las incumplen (el municipio 1927 de
 * Castellón no existe en el catálogo, por ejemplo) y una FK haría fallar la
 * ingesta diaria por un dato externo que no controlamos. La integridad se
 * garantiza validando el lote antes de escribirlo (ver `validarLote`).
 * Si añades `.references()` a una columna, estarás creando una bomba de
 * relojería para el cron de las 08:00.
 */
import {
  pgTable,
  text,
...
export const ccaa = pgTable("ccaa", {
...
export const provincias = pgTable(
  ...
  (table) => ({ idxCcaa: index("idx_provincias_ccaa").on(table.ccaaId) })
);
...
export const municipios = pgTable(
  ...
  (table) => ({ idxProvincia: index("idx_municipios_provincia").on(table.provinciaId) })
);
...
export const estaciones = pgTable("estaciones", {
  ...
  bioetanolPct: doublePrecision("bioetanol_pct").notNull().default(0),
  /** Porcentaje de éster metílico */
  esterMetilicoPct: doublePrecision("ester_metilico_pct").notNull().default(0),
  /** Fecha de última actualización (ISO 8601) */
  fechaActualizacion: text("fecha_actualizacion").notNull(),
},
  (table) => ({
    // Las páginas geográficas (CCAA → provincia → municipio) y el gate del
    // sitemap filtran/agrupan por estas tres columnas.
    idxMunicipio: index("idx_estaciones_municipio").on(table.municipioId),
    idxProvincia: index("idx_estaciones_provincia").on(table.provinciaId),
    idxCcaa: index("idx_estaciones_ccaa").on(table.ccaaId),
  })
);
...
export const precios = pgTable(
  ...
  (table) => ({
    ...
    // El sitemap y las páginas de combustible recorren los precios por
    // producto («¿qué estaciones tienen este combustible hoy?»).
    idxProducto: index("idx_precios_producto").on(table.productoId),
    idxFecha: index("idx_precios_fecha").on(table.fechaObservacion),
  })
);
 ...
 */
export const histGeoDia = pgTable(
  "hist_geo_dia",
  {
    ...
    precioMedio: doublePrecision("precio_medio").notNull(),
 ...
 */
export const histGeoSemana = pgTable(
  "hist_geo_semana",
  {
...
export const histMunMes = pgTable(
  "hist_mun_mes",
  {
...
export const histProvMes = pgTable(
  "hist_prov_mes",
  {
...
export const histCcaaMes = pgTable(
  "hist_ccaa_mes",
  {
...
export const histEstacionMes = pgTable(
  "hist_estacion_mes",
  {
    estacionId: text("estacion_id").notNull(),
    productoId: integer("producto_id").notNull(),
 ...
 */
export const histNacDia = pgTable(
  "hist_nac_dia",
  {
    productoId: integer("producto_id").notNull(),
    fecha: text("fecha").notNull(),
...
export const histMesesProcesados = pgTable("hist_meses_procesados", {
 ...
 */
export const resumenNacional = pgTable("resumen_nacional", {
  productoId: integer("producto_id").primaryKey(),
  precioMedio: doublePrecision("precio_medio").notNull(),
  precioMin: doublePrecision("precio_min").notNull(),
  precioMax: doublePrecision("precio_max").notNull(),
